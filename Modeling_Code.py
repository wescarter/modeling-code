...
#                  : Wesleyan University                           |
# MODIFIED         : 11/2/16 by Evan Carter                        |
...
#Read in & print RADMC-3D model parameters for a simple protoplanetary disk
...
#Create model, run radiative transfer simulation, generate initial
#image of the disk
...
#initialize all fitting parameters as an empty grid
#init_grid = [None] * len(vis)
model = [None] * len(vis)
data = [None] * len(vis)
datrlimwt = [None] * len(vis)
modrlimwt = [None] * len(vis)
openweights = [None] * len(vis)
weights = [None] * len(vis)
datrlxx = [None] * len(vis)
datrlyy = [None] * len(vis)
datimxx = [None] * len(vis)
datimyy = [None] * len(vis)
datrlI = [None] * len(vis)
datimI = [None] * len(vis)
modrlI = [None] * len(vis)
modimI = [None] * len(vis)
...
for i in range(len(model)):
    ...
    model[i].close
    os.system('fits in='+modelname+''+str(i)+'.fits op=xyin out='
               +modelname+''+str(i)+'.im')
    os.system('uvmodel model='+modelname+''+str(i)+'.im vis='+vis[i]+' options=replace out='
               +modelname+''+str(i)+'.vis')
    os.system('fits in='+modelname+''+str(i)+'.vis op=uvout out='
               +modelname+''+str(i)+'.uvf')


#Imaging convolved model & residuals
    os.system('uvmodel model='+modelname+''+str(i)+'.im vis='+vis[i]+' options=subtract out='
               +modelname+''+str(i)+'_residuals.vis')
    os.system('invert vis='+modelname+''+str(i)+'_residuals.vis map='+modelname+''+str(i)
               +'_residuals.mp beam='+modelname+''+str(i)+'_residuals.bm cell='+pixsize
               +' imsize=512 options=systemp,mfs robust=2')
    ...
    os.system('clean map='+modelname+''+str(i)+'.mp beam='+modelname+''+str(i)+'.bm out='
               +modelname+''+str(i)+'.cl  niters=10000') #cutoff=6.290E-05
    os.system('restor map='+modelname+''+str(i)+'.mp beam='+modelname+''+str(i)+'.bm model='
               +modelname+''+str(i)+'.cl out='+modelname+''+str(i)+'.cm')
...
for i in range(len(model)):
    ...
    datrlxx[i] = datrlimwt[i][:,0,0,0,0,0,0]
    datrlyy[i] = datrlimwt[i][:,0,0,0,0,1,0]
    datimxx[i] = datrlimwt[i][:,0,0,0,0,0,1]
    datimyy[i] = datrlimwt[i][:,0,0,0,0,1,1]
